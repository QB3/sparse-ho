...
labels = np.flip(["implicit_forward",  "implicit", "forward", "GridSearch", "bayesian", "random"])
...
for i in range(np.size(labels)):
    ...
    lines.append(axarr.flat[0].plot(df_temp['p'], df_temp['Error est'], label=dict_label[labels[i]], c=dict_color[labels[i]], markersize=dict_markers_size[labels[i]], marker=dict_markers[labels[i]], markevery=dict_markevery[i],fillstyle=dict_filling[labels[i]], markeredgewidth=2))
...
for i in range(np.size(labels)):
    df_temp = df_mean[df_mean['method'].eq(labels[i])]
    axarr.flat[1].semilogy(df_temp['p'], df_temp['time'], label=dict_label[labels[i]], c=dict_color[labels[i]], markersize=dict_markers_size[labels[i]], marker=dict_markers[labels[i]], markevery=dict_markevery[i],fillstyle=dict_filling[labels[i]], markeredgewidth=2)
...
labels = (["implicit_forward",  "implicit", "forward", "GridSearch", "bayesian", "random"])
...
labels = (['Imp. F. Iterdiff. (ours)', 'Implicit' , "F. Iterdiff.", "Grid-search", "Bayesian", "Random-search" ])
fig_legend = plt.figure(figsize=[18, 4])
fig_legend.legend([l[0] for l in lines], labels, ncol=3 , loc="upper center", fontsize=fontsize)
