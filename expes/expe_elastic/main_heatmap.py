...
for i in range(n_grid):
    for j in range(n_grid):
        clf = ElasticNet_sk(alpha=(alphas_1[i] + alphas_2[j]), fit_intercept=False,
                            l1_ratio=alphas_1[i] / (alphas_1[i] + alphas_2[j]),
                            tol=1e-7, max_iter=max_iter)
...
algo = ImplicitForward(criterion, tol_jac=1e-5, n_iter_jac=1000)
_, _, _ = grad_search(
...
ax.scatter3D(alphas_grad[:, 0], alphas_grad[:, 1], monitor.objs, c="red", s=60, marker="x")
fig.show()
