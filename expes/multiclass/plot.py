...
for idx, dataset_name in enumerate(dataset_names):
    ...
    for axarr in all_axarr:
        ...
        axarr.flat[idx].set_title("%s (K=%i)" % (dict_title[dataset_name], dict_n_classes[dataset_name]) )
...
if save_fig:
    for string, fig in zip(all_strings, all_figs):
        fig.savefig("%smulticlass_%s.pdf" % (fig_dir, string), bbox_inches="tight")
        fig.savefig("%smulticlass_%s.svg" % (fig_dir_svg, string), bbox_inches="tight")
    plot_legend_apart(
