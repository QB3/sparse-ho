...
class NNSVR(BaseModel):
    ...
    def _init_dbeta_ddual_var(self, X, y, dense0=None,
        ...
        ddual_var = np.zeros((2 * n_samples + n_features, 2))
        if jac0 is None or not compute_jac or self.ddual_var is None:
            dbeta = np.zeros((n_features, 2))
        else:
            if self.ddual_var.shape[0] != 2 * n_samples + n_features:
                ddual_var = np.zeros((2 * n_samples + n_features, 2))
            else:
    ...
    def _init_beta_dual_var(self, X, y, mask0, dense0):
        n_samples, n_features = X.shape
        dual_var = np.zeros(2 * n_samples + n_features)
    ...
    def _update_beta_jac_bcd(
        ...
        n_samples, n_features = X.shape
        for j in range(2 * n_samples + n_features):
            if j < (2 * n_samples):
                if j < n_samples:
                    j1, j2, sign = j, j, 1
                elif j >= n_samples:
    ...
    def _init_ddual_var(self, dbeta, X, y, sign_beta, hyperparam):
        ...
        bool_temp = np.isclose(dual_var, 0.0)
        sign[bool_temp] = -1.0
    ...
    def _update_only_jac(X, y, dual_var, dbeta, ddual_var,
        ...
        bool_temp = dual_var == 0.0
        gen_supp[bool_temp] = -1.0
    ...
    def _get_jac_t_v(self, X, y, jac, mask, dense, hyperparam, v, n_samples):
        ...
        hessian = np.concatenate((X[full_supp, :], -sub_id), axis=0)
    ...
    def proj_hyperparam(self, X, y, log_hyperparam):
        return np.clip(log_hyperparam, -16, [10, 10])
